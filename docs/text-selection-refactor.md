...
### Итерация отладки: схлопывание диапазона и «обгон» ручек

**Проблема (device-тест):** перетаскивание End-ручки левее Start схлопывало диапазон
в ноль и «подвешивало» выделение — ручки/тулбар исчезали, но `active` и `drag_started`
оставались `true`, блокируя тап-вне и повторное выделение до нового нажатия.

**Исправление 1 — сброс при схлопывании (первая итерация).** В `SelectionCore::drag_handle`
при `CCursorRange::is_empty()` вызывался `reset()` — это убирало «зависание» выделения
после схлопывания в один символ.

**Доработка — «обгон» ручек (ширший UX).** По запросу пользователя добавлено поведение
Android/текстовых редакторов: если тянущаяся капелька достигает пассивной и пересекает
её, роли меняются местами, и выделение НЕ схлопывается, а пересчитывается от новой
границы:

- Новое поле `SelectionCore.drag_side: Option<HandleSide>` — какую капельку физически
  тянут (первый кадр фиксирует; `primary`/`secondary` — роли, а не лево/право).
- В `drag_handle` активная капелька должна «оставаться on своей стороне»: End (primary)
  — правее (>=) Start, Start (secondary) — левее (<=) End. Как только активная граница
  **достигает** пассивной (`<=`/`>=`, включая равенство) — `swap(primary, secondary)` +
  разворот `drag_side`; дальше та же физическая капелька ведётся с другой стороны.
- **Точное совпадение (End дошла до Start) — не сброс, а «переворот»:** на этот кадр
  диапазон нулевой (пустой), но `active`/`drag_started`/`drag_side` сохраняются, и как
  только палец продолжит за пассивную границу — диапазон снова живой (обгон). Это
  исправляет сброс при совмещении капелек (наблюдался в device-логе: `selected=""`).
- **Сброс пустого при отпускании — на уровне виджета:** после `!any_down`, если
  `selection` всё ещё пуст (палец отпущен в точке совпадения, не доведя до обгона) —
  виджет вызывает `core.reset()`. Так «перевёртыш» без пальца не зависает.
- `reset()` обнуляет и `drag_side`.
- Согласованность с `dragged_handle`: ручки рисуются по `handle_positions`
  (sorted_cursors) ДО `drag_handle`; после обгона капелька под пальцем получает роль
  Start (по позиции), и `dragged_handle` возвращает Start — совпадает с `drag_side`.

Тесты: `drag_handle_end_meets_start_turns_handle_for_overtake` (совпадение → переворот,
дальше оживает), `drag_handle_end_overtakes_start_keeps_selection` и
