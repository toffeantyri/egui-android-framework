...
APP_LABEL="Counter"                     # Название (в списке приложений)
APP_PACKAGE="com.example.egui_counter"  # applicationId
APP_LIB_NAME="egui_android_counter"     # Имя .so (lib${APP_LIB_NAME}.so)
CRATE="egui-android-counter"            # Крейт с cdylib для cargo ndk
LOG_TAG="egui-counter"                  # Фильтр логов (adb logcat | grep)
