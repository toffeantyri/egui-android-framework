...
# ============================================================
# run_android.sh — сборка + установка + логи для showcase
#
...
#    из examples/showcase/AndroidManifest.xml с lib_name = egui_showcase
# 4. Собирает APK через ./gradlew assembleDebug
# 5. Устанавливает и запускает на устройстве
# 6. Показывает логи с фильтром egui-showcase
#
# Использование:
#   cd examples/showcase && ./run_android.sh
#   cd examples/showcase && ./run_android.sh --release
...
CRATE="egui-showcase"
LIB_NAME="lib${CRATE//-/_}.so"            # libegui_showcase.so
LOG_TAG="egui-showcase"                    # фильтр для adb logcat
...
export APP_LIB_NAME="${CRATE//-/_}"       # egui_showcase
export APP_PACKAGE="com.example.egui_showcase"
...
if [ "$INSTALL" = true ]; then
    ...
    echo "  Установлено."
    adb shell am start -n com.example.egui_showcase/com.example.egui_android.EguiActivity
fi
