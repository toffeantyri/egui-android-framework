//! Главный цикл для новой Decompose-style архитектуры.
//!
//! Использует `Application2` вместо старого `Application`.
//! Тот же EGL-бэкенд, input, lifecycle — другой рендеринг и диспатч.
...
pub fn run2<A: Application2>(app: AndroidApp) {
    ...
    log::info!("run2: запуск egui-android-framework (новая архитектура)");
    ...
    loop {
        ...
        if input_state.back_pressed {
            log::info!("Back нажата — выход (новая архитектура)");
            destroy_requested = true;
        }
        ...
        if destroy_requested {
            ...
            egl_state = None;
            log::info!("Выход из главного цикла (run2)");
            break;
        }
