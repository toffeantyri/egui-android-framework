//! Модификатор [`Fade`] — применяет прозрачность к виджету.
//!
//! Изолирует opacity через `ui.scope()`, чтобы не влиять
//! на другие виджеты после себя.

use std::marker::PhantomData;

use crate::{dispatcher::Dispatcher, widgets::Widget};

/// Модификатор прозрачности.
///
/// Применяет `ui.multiply_opacity(self.opacity)` к области
/// дочернего виджета. Значение `opacity` от 0.0 (прозрачный)
/// до 1.0 (непрозрачный).
///
/// # Пример
///
/// ```ignore
/// Text::new("Полупрозрачный").fade(0.5).render(ui, dispatch);
/// ```
pub struct Fade<W, M> {
    pub(crate) inner: W,
    pub(crate) opacity: f32,
    pub(crate) _marker: PhantomData<M>,
}

impl<W, M> Fade<W, M> {
    /// Создать Fade с заданной прозрачностью.
    ///
    /// `opacity` автоматически ограничивается диапазоном [0.0, 1.0].
    pub fn new(inner: W, opacity: f32) -> Self {
        Self {
            inner,
            opacity: opacity.clamp(0.0, 1.0),
            _marker: PhantomData,
        }
    }
}

impl<W: Widget<M>, M> Widget<M> for Fade<W, M> {
    fn render(&self, ui: &mut egui::Ui, dispatch: &Dispatcher<M>) {
        ui.scope(|ui| {
            ui.multiply_opacity(self.opacity);
            self.inner.render(ui, dispatch);
        });
    }
}
