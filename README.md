...
│  │  - Input │  │ ViewFn   │  │ Anim.  │  │ ChildS.││
│  │  - Loop  │  │ Lifecycle│  │ Theme  │  │ Lifecy.││
│  └────┬─────┘  └────┬─────┘  └───┬────┘  └────┬───┘│
│       └─────────────┼────────────┼─────────────┘    │
│                     ▼            ▼                   │
│            ┌──────────────────────────┐              │
...
- ✅ **Compose-like UI** — Column, Row, Stack, LazyColumn, модификаторы (.padding, .background, .border, .wrap_content, .fill_max_width), анимации, темы
- ✅ **Навигация** — ChildStack с управлением жизненным циклом экранов
- ✅ **Темы** — Material Design 3 (light/dark)
- ✅ **Кнопка Back** — иерархическая обработка: кастомная логика → ChildStack → завершение
...
|---|---|---|
| egui-android-core | [![crates.io](https://img.shields.io/crates/v/egui-android-core)](https://crates.io/crates/egui-android-core) | MVI примитивы: Component, ViewFn, LifecycleObserver, BackDispatcher |
...
## Обработка кнопки Back (полный пайплайн)

BackPressed обрабатывается в 4 крейтах, 7 шагов:

```
Android NDK (AKEYCODE_BACK)
  ↓
platform-android/input.rs: перехват → state.back_pressed = true, InputStatus::Handled
  ↓
platform-android/run.rs: app_instance.on_back_pressed()
  ↓
runtime/application.rs: trait Application::on_back_pressed()
  ↓
showcase/app.rs: self.root.on_back()
  ↓
RootComponent::on_back():
  ├── BackCustomScreen::handle_back()? — кастомная логика (переключение цвета)
  ├── NestedScreen::handle_back()? — pop из вложенного ChildStack
  └── ComponentContext::on_back()
      ├── BackDispatcher::handle() — зарегистрированные callback'и (диалоги)
      └── back_fallback — pop из корневого ChildStack или завершение
```

**Правила:**
- Platform **не знает** про Navigation — только флаг `back_pressed`
- Application **не принимает решений** — делегирует RootComponent::on_back()
- RootComponent проверяет активный экран на кастомный handle_back(), затем ComponentContext
- Любой экран может перехватить Back, вернув `true` из `handle_back()`
- Home — последний экран: Back на Home завершает приложение
