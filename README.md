...
- ✅ **Типобезопасные сообщения** — `Component::Message: Clone + Debug + Send + 'static`. `MessageEnvelope<M>` — типобезопасная обёртка. Ошибки downcast логируются с указанием ожидаемого типа.
...
└──────┬───────┘   └──────┬───────┘   └────────┬────────┘
       ...
       └──────────────────┼────────────────────┘
	      ...
	      │  (trait)     │◀──────│ run.rs — оркестратор      │
	      └──────────────┘       │ loop.rs — RunState + tick │
	                             │ graphics — Pipeline       │
	                             │ lifecycle — Init/Resume   │
	                             │ input_processing — Backend│
	                             │ system_bars, insets, ...  │
	                             └──────────────────────────┘
```
        ...
        → Component::handle(msg) — команда в data layer
          → Data Layer → store.update(|s| s.count += 1)
            → data_statechanged_tx.send(()) — сигнал Runtime
              → RuntimeContext::check() → UiNotifier::check() → request_repaint() + waker.wake()
