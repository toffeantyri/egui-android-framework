    ...
    fn begin(self, ui: &mut Ui) -> Prepared {
            ...
            && (state.content_is_too_large[0] || state.content_is_too_large[1])
        {
                ...
                .is_some_and(|response| response.dragged())
            {
                for d in 0..2 {
                    if scroll_enabled[d] {
                        // When user starts a new drag, we must reset the velocity
                        // from the previous fling animation. Without this, the old
                        // `state.vel` is applied on the first frame of the new drag,
                        // causing an instant jump of scroll_offset ("jank").
                        if content_response_option
                            .as_ref()
                            .is_some_and(|response| response.drag_started())
                        {
                            state.vel[d] = 0.0;
                            // Also clear smooth_scroll_delta when starting a new drag
