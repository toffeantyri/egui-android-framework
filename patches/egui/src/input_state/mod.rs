...
impl PointerState {
    ...
    pub(crate) fn begin_pass(
        ...
        // Сохраняем старую позицию для вычисления pointer.delta().
        // Если в этом кадре есть PointerButton { pressed: true } — обнуляем
        // old_pos, чтобы delta была нулевой на первом кадре нового drag.
        // Без этого delta = (new_pos - old_pos_from_UP) может быть огромной,
        // что вызывает мгновенный скачок scroll_offset.
        let mut old_pos = self.latest_pos;
        ...
        for event in &new.events {
            match event {
                ...
                Event::PointerButton {
                    ...
                    if pressed {
                        // Start of a drag: обнуляем old_pos, чтобы pointer.delta()
                        // на первом кадре нового drag была нулевой.
                        // Без этого delta = new_pos - old_pos_from_UP может быть
                        // огромной (десятки-сотни pt), вызывая скачок scroll_offset.
                        old_pos = Some(pos);
