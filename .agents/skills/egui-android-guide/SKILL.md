...
- `Button::new(text).on_click(msg)` — при клике диспатчит сообщение.
- `Text::new(text)` — отображает строку.
- `Spacer::new(size)` — вертикальный отступ.
- `Icon::new(image)` — отображает изображение.

### `Column<M>`, `Row<M>`, `Stack<M>`, `LazyColumn<M, T>` — `egui-android-ui`
- Контейнеры с `child()` builder pattern.
- Принимают `Box<dyn Widget<M>>` через `child()`.
- `Column` — вертикальное расположение, `Row` — горизонтальное, `Stack` — наложение, `LazyColumn` — скролл.
...
fn counter_view(state: &u32, ui: &mut egui::Ui, dispatch: &Dispatcher<Msg>) {
    ui.vertical_centered(|ui| {
        Column::<Msg>::empty()
            ...
            .child(Text::new("egui Counter"))
            .child(Spacer::new(16.0))
            .child(Text::new(format!("{}", state)).padding(16.0).background(egui::Color32::from_gray(40)))
            .child(Button::new("+1").on_click(Msg::Increment).padding(8.0).background(egui::Color32::from_rgb(0, 128, 255)))
            .render(ui, dispatch);
...
13. **Compose-like UI строится через:**
    ```rust
    // Колонка с дочерними виджетами
