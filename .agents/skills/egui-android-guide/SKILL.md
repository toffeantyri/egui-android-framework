...
Intent и Message — это одно и то же (`Msg`).
...
| Уровень | Механизм | Сохраняется при kill/restore | Примеры |
|---------|----------|------------------------------|---------|
| Бизнес-данные (MVI) | store.update() + reducer | ✅ Да | counter, список товаров, auth-токен |
| UI-состояние (Local) | remember() / on_click_with() | ❌ Нет | expanded, scroll position, текст ввода |

Правило: если данные нужны после пересоздания Activity — это бизнес-данные → MVI.
...
- Generic только по `C` (конфигурация/route). `C: Clone + PartialEq + Debug + Serialize + DeserializeOwned`.
...
- `save() -> SavedStack<C>` — сохранить конфигурации и состояния всех компонентов
  для восстановления после пересоздания Activity. Каждый компонент отдаёт
  `Vec<u8>` через `ComponentNode::save_state()`.
- `restore_from_saved(saved, factory)` — пересоздать весь стек из сохранённого
  состояния. Компоненты создаются через `factory.create(config)`, затем
  вызывается `ComponentNode::restore_state()`.
- **Критически важно (как в Decompose):** при восстановлении (`restore_from_saved`)
  все компоненты **создаются заново** через фабрику, а не обновляются по месту.
  Конфигурация (`Route::State`) определяет какой компонент создать.
...
### `PersistentComponent<T>` — `egui-android-core`
- Структурная обёртка для `Component + PersistentState`.
- Реализует `ComponentNode`, делегирует `render`/`handle_dyn`,
  и переопределяет `save_state`/`restore_state` через `PersistentState`.
- Используется в фабрике: `Box::new(PersistentComponent::new(MyScreen::new()))`.
- Решение конфликта blanket-impl: не blanket, а композиция.
...
- **Важно:** компонент нужно обернуть в `PersistentComponent<T>` в фабрике.
...
Сохранение/восстановление состояния работает на трёх слоях:

```
Слой 1: Application (Rust)
  └── saved_state: Option<Vec<u8>> — хранит сериализованный стек
      │
      Слой 2: ChildStack::save/restore
      │
      ├── Конфигурации всех элементов стека (Vec<C>)
      └── Состояния компонентов (Vec<Option<Vec<u8>>>)
          │
          Слой 3: ComponentNode::save_state/restore_state
          │
          ├── PersistentComponent<T> (через PersistentState::save_to_boxed)
...
InitWindow → app.on_restore_state(None)
  → bytes = self.saved_state.take()
  → root.restore(saved) → ChildStack::restore_from_saved()
```
...
2. В фабрике обернуть в `PersistentComponent`:

```rust
Route::MyScreen => Box::new(PersistentComponent::new(MyScreen::new())),
```

3. Для экранов с вложенными стеками (NestedScreen) — impl ComponentNode напрямую:
...
- Обычные компоненты без `PersistentComponent<T>` (HomeScreen, WidgetsScreen etc.)

### Платформа не хранит состояние

`platform-android` не содержит `saved_state` в `RunState`.
Application владеет состоянием, Platform только передаёт lifecycle.
Это архитектурный контракт: Platform не знает про состояние приложения.
...
предотвращает kill процесса. `Application` хранит `saved_state` в своём поле
и восстанавливает при InitWindow. Platform не хранит состояние.
...
6. **`store.update()` — единственная точка изменения БИЗНЕС-состояния.**
   Локальное UI-состояние (`remember()` / `on_click_with()`) — исключение.
