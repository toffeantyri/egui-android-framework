...
Проект использует **гибридную модель**: MVI для бизнес-данных + локальное UI-состояние.

## Уровень 1: MVI / бизнес-данные

Любое изменение бизнес-данных происходит только через поток:
...
- Данные сохраняются при kill/restore через PersistentState.
...
- `remember()` и `on_click_with()` мутируют локальное состояние напрямую.
- Состояние НЕ сохраняется при kill/restore.
- Примеры: expanded/collapsed, позиция скролла, текст ввода.

## Правило выбора

Если данные нужны после пересоздания Activity — это бизнес-данные → MVI.
Если нет — UI-состояние → remember().

## Аналогия с Jetpack Compose

| Компонент | egui-android-framework | Jetpack Compose |
|-----------|----------------------|-----------------|
| Бизнес-состояние | store.update() + reducer | ViewModel + StateFlow |
| UI-состояние | remember() | remember() / rememberSaveable() |

---

### Диаграмма гибридной модели
...
**Экраны с внутренним `ChildStack` (NestedScreen, Layer2Screen):**

Они владеют собственным стеком, поэтому `handle_dyn`/`handle_back` остаются
ручными, но дублирование между такими экранами устранено через `ChildStack`:
- `delegate_back(ctx)` — рекурсивно делегирует Back активному компоненту
  (поп при `Pop`/`Propagate`, проброс `Handled`/`Finish`, `Propagate` на пустом цикле)
- `delegate_dyn(msg, ctx)` — пробрасывает чужое сообщение активному подэкрану

Один раз написанная логика контейнера, переиспользуется всеми экранами-хостами.
