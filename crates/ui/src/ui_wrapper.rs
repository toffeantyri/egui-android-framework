...
//! # Constraints storage
//!
//! Constraints хранятся в двух местах (гибридный подход):
//!
//! 1. **Поле `constraints`** — type-safe, быстрый доступ, без lookup.
//! 2. **`Context::data()`** — constraints записываются с ключом
//!    `ui.id().with("ui_wrapper_cx")`, чтобы пережить `Frame::show`,
//!    `ScrollArea::show` и другие egui-обёртки.
//!
//! При создании `UiWrapper::new(ui, constraints)` — constraints
//! записываются в оба места. При создании `UiWrapper::new_unconstrained(ui)`
//! — читаются из Context (если были установлены родителем), иначе default.
...
///
/// Используем фиксированный Id, а не `ui.id().with(...)`, чтобы
/// constraints переживали `Frame::show`, `ScrollArea::show` и другие
/// egui-обёртки, которые создают новый Ui с другим id.
fn cx_key() -> egui::Id {
...
impl<'a> UiWrapper<'a> {
    ...
    /// Constraints записываются в Context перед созданием child.
...
mod tests {
    ...
    fn test_new_child_via_frame_keeps_constraints() {
        // Проверяет что constraints переживают Frame::show
        with_ui(|ui| {
            let constraints = Constraints::exact(200.0, 100.0);
            let mut wrapper = UiWrapper::new(ui, constraints);
            ...
            egui::Frame::NONE
                ...
                .show(&mut *wrapper, |inner_ui| {
                    // Создаём UiWrapper внутри Frame
                    let inner = UiWrapper::new_unconstrained(inner_ui);
                    // Должны прочитать constraints из Context
                    assert_eq!(
    ...
    fn test_set_constraints() {
        with_ui(|ui| {
            ...
            // Проверяем что constraints записались в Context
            let read_back = read_cx(&*wrapper);
            assert_eq!(read_back.min_width, 100.0);
        });
    }
